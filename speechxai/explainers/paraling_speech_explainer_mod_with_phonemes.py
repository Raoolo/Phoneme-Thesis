...
class ParalinguisticSpeechExplainer:
    ...
    def plot_phoneme_importance_heatmap(self, features, scores, perturbation_values, target):
        """
        Plot a heatmap of phoneme importance scores across perturbation values.
        """
    ...
    def plot_perturbation_trends(self, features, scores, perturbation_values, target):
        """
        Plot trend lines showing how phoneme importance changes with perturbation strength.
        """
        plt.figure(figsize=(10, 6))
    ...
    def plot_phoneme_importance_bar(self, features, scores, labels):
        """
        Plot a bar chart of phoneme importance scores.
        """
    ...
    # def time_stretch_phoneme(self, audio_segment: np.ndarray, stretch_factor: float, sample_rate: int) -> np.ndarray:
    #     """
    #     Apply time stretching to a phoneme segment using pyrubberband.
    #     """
    #     return time_stretch(y=audio_segment, sr=sample_rate, rate=stretch_factor)
    ...
    def compute_explanation(
        ...
        if phonemization or word_level:
            ...
            self.plot_phoneme_importance_bar(features, scores, target_classes_show)
        ...
        if complete_perturbation:
            if word_level or phonemization:
                ...
                for label_idx, target in enumerate(target_classes_show):
                    ...
                    self.plot_perturbation_trends(features, scores_all[label_idx], perturbations_all, target)
                    self.plot_phoneme_importance_heatmap(features, scores_all[label_idx], perturbations_all, target)
