...
class LimeTimeSeriesExplainer(object):
    ...
    def __init__(
        ...
        Args:
            ...
            signal_names: list of strings, names of signals
            phonemization: if true, working at phoneme level instead of word level
        """

        # exponential kernel
        def kernel(d):
            # the kernel width control the "locality" of the explanation (smaller --> closer/similar samples)
            # so for phoneme level we might want to change the value from 25 to something else
    ...
    def explain_instance(
        ...
        splits=None,  # list of dicts with start, end and word.
    ):
        ...
        Args:
            ...
            model_regressor: sklearn regressor to use in explanation. Defaults
                ...
                model_regressor.fit()
            splits: list of dicts with start, end and word/phoneme. # MODIFIED
        Returns:
        ...
        for label in labels:
            (
                ...
                ret_exp.score,      #  R^2 value of the returned explanation
                ret_exp.local_pred,
        ...
        # print(f"ret_exp: {ret_exp}")
        return ret_exp

    # import Levenshtein

    # def compute_per(original_sequence, perturbed_sequence):
    #     # Compute Levenshtein distance
    #     distance = Levenshtein.distance(original_sequence, perturbed_sequence)
    #
    #     # Total phonemes in the original sequence
    #     total_phonemes = len(original_sequence)
    #
    #     # PER = (Insertions + Deletions + Substitutions) / Total Phonemes
    #     per = distance / total_phonemes
    #     return per
    ...
    def __data_labels_distances_word_splits(
        ...
        # a different distance is dynamic time warping, used to compare temporal sequences which may vary in speed
        # pip install dtw-python
        # from dtw import dtw
        # distances = []
        # for i in range(1, len(x)):
        #     alignment = dtw(x[0], x[i], keep_internals=True)
        ...
        for i, num_inactive in enumerate(deact_per_sample, start=1):
            ...
            logging.info("sample %d, perturbing signals %r", i, channels_to_perturb)
            for idx in inactive_idxs:
                logging.info("sample %d, inactivating phoneme %r", i, splits[idx]["phoneme"])
