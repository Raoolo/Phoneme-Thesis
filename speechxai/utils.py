...
def load_dataset_and_model(dataset_name, env='colab', data_dir=None, model_dir=None, model_name=None):

   # Set default data directories based on environment
    if data_dir is None:
        if env == "colab":
            data_dir = "/content/drive/MyDrive/Tesi"
        elif env == "kaggle":
    ...
    if dataset_name == "FSC":
        ## Load audio
        if env == "colab":
            data_path = f"{data_dir}/FSC/fluent_speech_commands_dataset"
        elif env == "kaggle":
            data_path = f"{data_dir}/fsc-dataset/FSC/fluent_speech_commands_dataset"
        else:
            data_path = f"{data_dir}/data/speech/fluent_speech_commands_dataset"
        ...
        if env == "colab":
            data_path = f"{data_dir}/IEMOCAP"
        elif env == "kaggle":
            data_path = f"{data_dir}/iemocap-dataset/IEMOCAP"
        else:
            data_path = f"{data_dir}/IEMOCAP"
        ...
        if os.path.basename(data_path) == "IEMOCAP":
            data_path = data_path.replace(f"{os.path.sep}IEMOCAP", "")

        # extracts metadata from directory
        dataset_ta = torchaudio.datasets.IEMOCAP(data_path, sessions=[1])
        data_path = data_path + f"{os.path.sep}IEMOCAP"
        ...
        df = df[["path","label"]]
        df["path"] = df["path"].apply(lambda x: os.path.join(data_path, x))
        dataset = Dataset.from_pandas(df)
        ...
        if data_dir is None:
            if env == "colab":
                data_dir = f"{data_dir}/ITALIC"
            elif env == "kaggle":
                data_dir = f"{data_dir}/italic-dataset/ITALIC"
            else:
                data_dir = f"{str(Path.home())}/ITALIC"

        ## Load model and feature extractor
        model_checkpoint = (
            data_dir
            + "/"
            + "wav2vec2-xls-r-300m-ic-finetuning-hard-speaker/s"
        )
